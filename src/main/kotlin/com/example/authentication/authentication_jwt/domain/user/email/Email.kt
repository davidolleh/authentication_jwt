...
data class Email(var email: String) : Contact {
    init {
        if(!isInEmailAddressFormat(email)) {
            throw EmailInvalidFormatException(email=email)
        }
    }

    override fun destination(): String = email
    ...
    companion object {
         ...
         */
        fun isInEmailAddressFormat(email : String) : Boolean =
            email.length <= 320 && Regex(emailPattern).matches(email)
    }
